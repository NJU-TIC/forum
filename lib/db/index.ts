...
function normalizeGameScores(
  user: Document | null | undefined,
): Record<string, number> {
  if (!user || !user.gameScores || typeof user.gameScores !== "object") {
    return {};
  }

  return Object.entries(user.gameScores as Record<string, unknown>).reduce<
    Record<string, number>
  >((acc, [gameId, rawScore]) => {
    if (
...
export async function createUser(userData: unknown): Promise<SUser> {
  ...
  const validatedUser = validateUser({
    ...userInput,
    gameScores:
      userInput.gameScores &&
      typeof userInput.gameScores === "object" &&
      !Array.isArray(userInput.gameScores)
        ? userInput.gameScores
        : {},
  });
...
export async function getUserGameScoreSummary(userId: string): Promise<{
  ...
  const gameScores = normalizeGameScores(user);
...
export async function getGameTotalScores(
  gameIds?: string[],
  ...
  const pipeline: Document[] = [
    {
      $project: {
        scoreEntries: {
          $objectToArray: { $ifNull: ["$gameScores", {}] },
        },
      },
    },
    { $unwind: "$scoreEntries" },
